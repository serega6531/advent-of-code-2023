...
fun main() {
    ...
    val lineOffsets = calculateOffsets(emptyLines)
    val columnOffsets = calculateOffsets(emptyColumns)

    val galaxies: List<YX> = (0..maxY).cartesianProduct(0..maxX).filter { (y, x) -> galaxiesXY[y][x] }
...
private fun calculateOffsets(presence: Array<Boolean>) = presence.runningFold(0) { acc, empty ->
    if (empty) {
        acc + 2
    } else {
        acc + 1
